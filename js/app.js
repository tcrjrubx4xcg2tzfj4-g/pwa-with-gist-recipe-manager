    ...
    try {
        ...
        if (remoteContent) {
            try {
                ...
                remoteData = parsed.recipes ? parsed : { recipes: parsed, lastUpdated: null };
            } catch (e) {
        ...
        if (recipes.length === 0 && remoteRecipes.length > 0) {
            ...
            const remoteLastUpdated = remoteData.lastUpdated
                ...
                : (remoteRecipes.length > 0
                    ? Math.max(...remoteRecipes.map(r => new Date(r.updatedAt || r.createdAt).getTime()))
                    : 0);
    ...
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', () => {
            navigator.serviceWorker.register('/service-worker.js')
                .then((registration) => {
                    console.log('[App] ServiceWorker registration successful:', registration);
                })
                .catch((error) => {
